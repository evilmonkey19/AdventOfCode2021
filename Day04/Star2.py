...
def playBingo(numbers : list, cardboards : list) -> Tuple: 
    for n in numbers:
        extractAllDoneCardboardsExceptIfLast(cardboards)
        cardboards = markNumberOnCardboards(n, cardboards)
        if (len(cardboards) == 1 and len(isThereWinners(cardboards)) > 0):
            return (n, cardboards[0])
    return (-1,-1)

def extractAllDoneCardboardsExceptIfLast(cardboards : list) -> None:
    if(len(winners:=isThereWinners(cardboards))) >= 0:
        for winner in reversed(winners):
            if len(cardboards) == 1 :
                return
            cardboards.pop(winner)
...
def isThereWinners(cardboards : list) -> list:
    res = []
