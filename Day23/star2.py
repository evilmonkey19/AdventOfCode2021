...
def getPieceFromRoom(room):
    return next((c for c in room if c != '.'), None)
...
def canReach(board, start_pos, end_pos):
    ...
    for pos in range(a,b+1):
        ...
        if pos in ROOM_POSITIONS: continue
        if board[pos] != '.': 
            return False
    return True
...
def possibleMoves(board, pos):
    ...
    for dest in range(len(board)):
        ...
        if dest in ROOM_POSITIONS and ROOMS[movingLetter] != dest: continue
...
def move(board, pos, dest):
    ...
    if len(board[dest]) == 1:
        ...
        new_board[dest], new_dist = addToRoom(movingLetter, board[dest])
        dist += new_dist
...
def solve(board):
    ...
    while queue:
        ...
        for pos, piece in enumerate(board):
            if getPieceFromRoom(piece) is None: continue
            dests = possibleMoves(board, pos)
            for dest in dests:
                new_board, add1_cost = move(board, pos, dest)
                new_cost = states[tuple(board)] + add1_cost
